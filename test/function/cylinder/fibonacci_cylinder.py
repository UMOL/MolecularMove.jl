...
def julia_matrix_format(matrix):
    def bracket(left, content, right):
        return "{}{}{}".format(left, content, right)
    def stringify(array):
    ...
    output = []
    for i in range(m):
        output.append(" ".join(stringify(matrix[i, :])))
    return bracket("[", ";\n".join(output), "]")
...
def main():
    ...
    zmax = 5
    N = 5
    data = np.array(gen_data(N, R, zmin, zmax))
    np.set_printoptions(precision=15)
    print(julia_matrix_format(data))
